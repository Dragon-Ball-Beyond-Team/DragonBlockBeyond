...
public class RecipeButton extends AbstractWidget {
   ...
   public void renderButton(PoseStack p_100484_, int p_100485_, int p_100486_, float p_100487_) {
      ...
      this.blit(p_100484_, this.getX(), this.getY(), i, j, this.width, this.height);
      ...
      if (this.collection.hasSingleResultItem() && this.getOrderedRecipes().size() > 1) {
         minecraft.getItemRenderer().renderAndDecorateItem(itemstack, this.getX() + k + 1, this.getY() + k + 1, 0, 10);
         --k;
      }

      minecraft.getItemRenderer().renderAndDecorateFakeItem(itemstack, this.getX() + k, this.getY() + k);
      if (flag) {
         posestack.popPose();
         RenderSystem.applyModelViewMatrix();
      }
