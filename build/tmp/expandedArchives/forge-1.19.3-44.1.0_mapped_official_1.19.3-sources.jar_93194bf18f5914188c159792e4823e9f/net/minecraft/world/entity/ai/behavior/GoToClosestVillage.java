   ...
   public static BehaviorControl<Villager> create(float p_260342_, int p_259691_) {
      return BehaviorBuilder.create((p_258357_) -> {
         return p_258357_.group(p_258357_.absent(MemoryModuleType.WALK_TARGET)).apply(p_258357_, (p_258366_) -> {
            return (p_258361_, p_258362_, p_258363_) -> {
               if (p_258361_.isVillage(p_258362_.blockPosition())) {
                  return false;
               } else {
                  PoiManager poimanager = p_258361_.getPoiManager();
                  int i = poimanager.sectionsToVillage(SectionPos.of(p_258362_.blockPosition()));
                  Vec3 vec3 = null;

                  for(int j = 0; j < 5; ++j) {
                     Vec3 vec31 = LandRandomPos.getPos(p_258362_, 15, 7, (p_147554_) -> {
