...
public class SummonCommand {
   ...
   private static int spawnEntity(CommandSourceStack p_249752_, Holder.Reference<EntityType<?>> p_251948_, Vec3 p_251429_, CompoundTag p_250568_, boolean p_250229_) throws CommandSyntaxException {
      BlockPos blockpos = new BlockPos(p_251429_);
      if (!Level.isInSpawnableBounds(blockpos)) {
         throw INVALID_POSITION.create();
      } else {
         CompoundTag compoundtag = p_250568_.copy();
         compoundtag.putString("id", p_251948_.key().location().toString());
         ServerLevel serverlevel = p_249752_.getLevel();
         Entity entity = EntityType.loadEntityRecursive(compoundtag, serverlevel, (p_138828_) -> {
            p_138828_.moveTo(p_251429_.x, p_251429_.y, p_251429_.z, p_138828_.getYRot(), p_138828_.getXRot());
            return p_138828_;
         });
         if (entity == null) {
            throw ERROR_FAILED.create();
         } else {
            if (p_250229_ && entity instanceof Mob) {
               ((Mob)entity).finalizeSpawn(p_249752_.getLevel(), p_249752_.getLevel().getCurrentDifficultyAt(entity.blockPosition()), MobSpawnType.COMMAND, (SpawnGroupData)null, (CompoundTag)null);
            }
