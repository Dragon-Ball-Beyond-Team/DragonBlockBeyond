...
public class PlaceCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> p_214548_) {
      ...
      }).then(Commands.literal("feature").then(Commands.argument("feature", ResourceKeyArgument.key(Registries.CONFIGURED_FEATURE)).executes((p_248164_) -> {
         return placeFeature(p_248164_.getSource(), ResourceKeyArgument.getConfiguredFeature(p_248164_, "feature"), new BlockPos(p_248164_.getSource().getPosition()));
      ...
      })))).then(Commands.literal("jigsaw").then(Commands.argument("pool", ResourceKeyArgument.key(Registries.TEMPLATE_POOL)).then(Commands.argument("target", ResourceLocationArgument.id()).then(Commands.argument("max_depth", IntegerArgumentType.integer(1, 7)).executes((p_248165_) -> {
         return placeJigsaw(p_248165_.getSource(), ResourceKeyArgument.getStructureTemplatePool(p_248165_, "pool"), ResourceLocationArgument.getId(p_248165_, "target"), IntegerArgumentType.getInteger(p_248165_, "max_depth"), new BlockPos(p_248165_.getSource().getPosition()));
      ...
      })))))).then(Commands.literal("structure").then(Commands.argument("structure", ResourceKeyArgument.key(Registries.STRUCTURE)).executes((p_248166_) -> {
         return placeStructure(p_248166_.getSource(), ResourceKeyArgument.getStructure(p_248166_, "structure"), new BlockPos(p_248166_.getSource().getPosition()));
   ...
   public static int placeStructure(CommandSourceStack p_214588_, Holder.Reference<Structure> p_251799_, BlockPos p_214590_) throws CommandSyntaxException {
      ...
      if (!structurestart.isValid()) {
         ...
         ChunkPos.rangeClosed(chunkpos, chunkpos1).forEach((p_248162_) -> {
            structurestart.placeInChunk(serverlevel, serverlevel.structureManager(), chunkgenerator, serverlevel.getRandom(), new BoundingBox(p_248162_.getMinBlockX(), serverlevel.getMinBuildHeight(), p_248162_.getMinBlockZ(), p_248162_.getMaxBlockX(), serverlevel.getMaxBuildHeight(), p_248162_.getMaxBlockZ()), p_248162_);
         });
