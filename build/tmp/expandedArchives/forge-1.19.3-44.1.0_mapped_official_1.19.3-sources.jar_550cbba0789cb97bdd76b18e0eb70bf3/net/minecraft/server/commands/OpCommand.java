...
public class OpCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> p_138080_) {
      p_138080_.register(Commands.literal("op").requires((p_138087_) -> {
         ...
         return SharedSuggestionProvider.suggest(playerlist.getPlayers().stream().filter((p_253477_) -> {
            return !playerlist.isOp(p_253477_.getGameProfile());
         }).map((p_253478_) -> {
            return p_253478_.getGameProfile().getName();
         }), p_138085_);
      }).executes((p_138082_) -> {
