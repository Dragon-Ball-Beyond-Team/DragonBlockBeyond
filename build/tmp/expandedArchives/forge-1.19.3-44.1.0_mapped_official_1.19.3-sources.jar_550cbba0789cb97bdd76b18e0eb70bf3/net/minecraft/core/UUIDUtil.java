...
public final class UUIDUtil {
   ...
   public static final Codec<UUID> STRING_CODEC = Codec.STRING.comapFlatMap((p_261406_) -> {
      try {
         return DataResult.success(UUID.fromString(p_261406_), Lifecycle.stable());
      } catch (IllegalArgumentException illegalargumentexception) {
         return DataResult.error("Invalid UUID " + p_261406_ + ": " + illegalargumentexception.getMessage());
      }
   }, UUID::toString);
   public static Codec<UUID> AUTHLIB_CODEC = Codec.either(CODEC, Codec.STRING.comapFlatMap((p_253363_) -> {
      try {
         return DataResult.success(UUIDTypeAdapter.fromString(p_253363_), Lifecycle.stable());
      } catch (IllegalArgumentException illegalargumentexception) {
         return DataResult.error("Invalid UUID " + p_253363_ + ": " + illegalargumentexception.getMessage());
      }
